    ...
    return (<div> 
            <div className='mb-20'>
                <div className='bg-blue-100'>
                    <h1 className='mt-10 mb-4 text-4xl font-bold leading-none tracking-tight text-gray-900 md:text-5xl lg:text-6xl dark:text-white'> פרטי מטופל {customerCoronaDetails?.name}</h1>
                </div>
            <div className='text-right'> {customerCoronaDetails?.imageUrl && <img style={{width: "100px", height: "100%"}} src={customerCoronaDetails?.imageUrl} alt="Uploade" />} </div>
            ...
            <div><ImageUpload customerId={customerCoronaDetails.id} setImageUploaded={setImageUploaded}/></div>
            </div>
        </div>
    );
}
